# Port-forward the UI services from kind-preprod.
...
#   shop-qa-ui         → http://localhost:8501
# ---------------------------------------------------------------------------
...
Write-Host "  shop-qa-ui         -> http://localhost:8501" -ForegroundColor Green
...
$jobs += Start-Job -Name "pf-shop-qa-ui" -ScriptBlock {
    kubectl --context $using:context -n $using:namespace port-forward svc/shop-qa-ui 8501:80
}

$jobs += Start-Job -Name "pf-grafana" -ScriptBlock {
    kubectl --context $using:context -n $using:namespace port-forward svc/grafana 3002:3000
}
